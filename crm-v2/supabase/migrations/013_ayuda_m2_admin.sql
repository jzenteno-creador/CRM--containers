-- ============================================================================
-- 013_ayuda_m2_admin — M2 rebuild v2 CRM Detention
-- Versiona en el canal único de migraciones el seed generado por el ui-builder
-- en crm-v2/supabase/seeds-ayuda/m2_admin.sql (contenido idéntico).
--
-- Revisión del schema-builder antes de aplicar:
--   * Solo DML sobre crm.ayuda_contenido (sección `admin`) — dentro del
--     perímetro §21.
--   * Idempotente por patrón delete+insert acotado a seccion='admin'. El
--     DELETE corre como rol de migración (postgres), NO viola "sin DELETE
--     desde la app": authenticated sigue sin grant de DELETE (002/§14.5).
--   * Verificado en vivo pre-aplicación: la sección `admin` tenía 0 filas
--     (solo existía `faq` con 6 filas del seed 008) ⇒ el DELETE es no-op
--     defensivo en esta corrida.
-- ============================================================================
