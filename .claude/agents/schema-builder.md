...
8. RPCs operativas = SECURITY INVOKER (RLS aplica adentro). **Lista CERRADA de SECURITY DEFINER** (plan M0+M1 aprobado + addendum M2): `perfil`, `mi_estado_cuenta` (M2, migración 012 — devuelve SOLO `estado_cuenta`+`rechazo_motivo` de la fila PROPIA del caller; §12.3 exige que el rechazado vea el motivo y §14.3 impide resolverlo vía policy), `aprobar_usuario`, `rechazar_usuario`, `set_estado_usuario`, `get_pendientes`, `crm_nueva_version_freetime`, `crm_validar_reforzado`, `sync_mi_usuario` (M2, migración 016 — auto-reparación de la fila espejo PROPIA del caller: crea solo pendiente/rol NULL desde auth.users, sin escalación), y TODAS las funciones de trigger que escriben en otra tabla (`handle_new_user`, `bootstrap_admin`, timeline, `planta_actual` — los triggers corren con los privilegios del que dispara, NO del owner; los de `auth.users` son además DEFENSIVOS por migración 014: capturan TODA excepción y jamás la propagan, porque `auth.users` es compartida por el proyecto). Toda DEFINER: `SET search_path` pinneado + primera línea guard `perfil().estado='activo'` (+ check de rol) — excepciones documentadas al guard: `perfil`, `mi_estado_cuenta` y `sync_mi_usuario` (su propósito ES servir a cuentas no activas / recién creadas).
