...
8. RPCs operativas = SECURITY INVOKER (RLS aplica adentro). **Lista CERRADA de SECURITY DEFINER** (plan M0+M1 aprobado + addendum M2): `perfil`, `mi_estado_cuenta` (M2, migración 012 — devuelve SOLO `estado_cuenta`+`rechazo_motivo` de la fila PROPIA del caller; §12.3 exige que el rechazado vea el motivo y §14.3 impide resolverlo vía policy), `aprobar_usuario`, `rechazar_usuario`, `set_estado_usuario`, `get_pendientes`, `crm_nueva_version_freetime`, `crm_validar_reforzado`, y TODAS las funciones de trigger que escriben en otra tabla (`handle_new_user`, timeline, `planta_actual` — los triggers corren con los privilegios del que dispara, NO del owner). Toda DEFINER: `SET search_path` pinneado + primera línea guard `perfil().estado='activo'` (+ check de rol) — excepciones documentadas al guard: `perfil` y `mi_estado_cuenta` (su propósito ES servir a cuentas no activas).
